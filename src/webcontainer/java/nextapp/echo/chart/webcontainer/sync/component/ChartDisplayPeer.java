...
public class ChartDisplayPeer extends AbstractComponentSynchronizePeer {
	...
	addEvent(new AbstractComponentSynchronizePeer.EventPeer("action",
		ChartDisplay.ACTION_LISTENERS_CHANGED_PROPERTY) {
	...
	if (propertyName.equals(PROPERTY_URI)) {
	    return ChartImageService.getInstance()
		    .createUri(WebContainerServlet.getActiveConnection().getUserInstance(), chartDisplay);
	...
	if (update.hasUpdatedProperty(ChartDisplay.PROPERTY_HEIGHT)
	    ...
	    return new MultiIterator(new Iterator[] { normalPropertyIterator,
		    new ArrayIterator(new String[] { PROPERTY_URI, PROPERTY_IMAGE_MAP }) });
	} else {
     ...
     */
    public void storeInputProperty(Context context, Component component, String propertyName, int propertyIndex,
	    Object newValue) {
